...
- artifact manifest 是正式产物上传的唯一清单来源；只上传其中明确登记的文件，不扫描工作区或输出目录。

### 正式产物

- 上传 manifest 中明确登记的 `kind=file` 正式产物；
- 阶段结束增量同步，Run 结束再次核对；
- 第一版单文件上限 20 MiB，整文件幂等重试；
- 不上传中间章节、日志、临时文件、未登记附件或外部 URI 内容；
- 暂不解析正文、不做在线预览、全文搜索和质量评价。
