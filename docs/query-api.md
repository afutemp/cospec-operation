...
除下述知识库查询明细外，普通查询 API 不返回会话消息正文、工具参数、工具输出、原始 JSONL、`path_hint` 或服务端绝对路径。原始 JSONL 只能通过上述管理员下载接口作为附件取得；页面不提供在线正文预览。

## 知识库消费汇总

```http
GET /api/v1/summaries/knowledge?from=2026-09-01T00:00:00Z&to=2026-09-07T23:59:59Z
```

返回查询次数、涉及 Run 数、命中/引用/告警总数，以及按执行状态、可回答性、知识库、发布版本、内容 revision、查询来源和消费 Skill 的分布。`items` 用于运营下钻；每项的 `detail` 保存 Query Adapter 本次实际处理的原始问题、完整回答、知识库位置、检索方式、命中文档与摘录、引用、告警、回答义务、覆盖和冲突信息。旧事件没有完整记录时 `detail` 为 `null`。

执行状态包括正常完成、降级完成、失败和记录不完整；可回答性只描述知识是否足以回答，不等同于执行是否成功。
