...
test("Run replay switches atomically on completed_with_errors and keeps old version on failure", async () => {
  ...
  const successRegistry = new ParserRegistry({ "0.4.0": (bytes) => parseCodexJsonl(bytes, "0.4.0") });
  const service = new ReplayService(repository, successRegistry);
  const completed = await service.replayRun(runId, "0.4.0");
  assert.equal(completed.status, "completed");
  assert.equal(repository.activeParserVersion(runId), "0.4.0");
  assert.deepEqual(await service.replayRun(runId, "0.4.0"), completed);

  let attempts = 0;
  const failureRegistry = new ParserRegistry({ "0.5.0": (bytes) => {
    attempts += 1;
    if (attempts === 2) throw new Error("parser_failed");
    return parseCodexJsonl(bytes, "0.5.0");
  } });
  const failed = await new ReplayService(repository, failureRegistry).replayRun(runId, "0.5.0");
  assert.equal(failed.status, "failed");
  assert.equal(failed.completed_chunks, 1);
  assert.equal(repository.activeParserVersion(runId), "0.4.0");
  assert.deepEqual(await new ReplayService(repository, failureRegistry).replayRun(runId, "0.5.0"), failed);
