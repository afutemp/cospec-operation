...
const filters = reactive({ dates: [] as string[], agentType: "", agentVersion: "", cospecPluginVersion: "" });
...
const queryFilters = computed(() => Object.fromEntries(Object.entries({
  ...
  agentType: filters.agentType, agentVersion: filters.agentVersion,
  cospecPluginVersion: filters.cospecPluginVersion,
...
const agentVersionOptions = computed(() => Object.keys(data.value.runs?.byAgentVersion ?? {}).map((key) => {
  const separator = key.indexOf("@");
  return { value: separator >= 0 ? key.slice(separator + 1) : key, label: key };
}).filter((option, index, values) => values.findIndex((item) => item.value === option.value) === index));
const cospecVersionOptions = computed(() => Object.keys(data.value.cospecPluginVersions?.byVersion ?? {}).filter((value) => value !== "<missing>"));
...
function reset() { rangePreset.value = "week"; filters.dates = []; filters.agentType = ""; filters.agentVersion = ""; filters.cospecPluginVersion = ""; }
</script>
<template><div><header class="page-head"><div><h1>运营总览</h1><p>查看 Cospec 工作流的使用规模和资源分布</p></div><div class="refresh"><span class="muted">更新于 {{ query.dataUpdatedAt.value ? new Date(query.dataUpdatedAt.value).toLocaleTimeString('zh-CN', {hour12:false}) : '—' }}</span><el-button :loading="query.isFetching.value" @click="query.refetch()">刷新数据</el-button></div></header>
  <section class="panel filters"><el-select v-model="rangePreset" aria-label="时间范围" style="width:135px"><el-option label="今天" value="today"/><el-option label="本周" value="week"/><el-option label="本月" value="month"/><el-option label="最近 7 天" value="last7"/><el-option label="最近 30 天" value="last30"/><el-option label="最近 90 天" value="last90"/><el-option label="自定义" value="custom"/></el-select><el-date-picker v-if="rangePreset === 'custom'" v-model="filters.dates" type="daterange" value-format="YYYY-MM-DD" start-placeholder="开始日期" end-placeholder="结束日期" /><el-select v-model="filters.agentType" clearable placeholder="Agent 类型" style="width:150px"><el-option label="Codex" value="codex"/><el-option label="Claude Code" value="claude_code"/></el-select><el-select v-model="filters.agentVersion" filterable clearable placeholder="Agent 版本" style="width:190px"><el-option v-for="option in agentVersionOptions" :key="option.label" :label="option.label" :value="option.value"/></el-select><el-select v-model="filters.cospecPluginVersion" filterable clearable placeholder="Cospec 版本" style="width:175px"><el-option v-for="version in cospecVersionOptions" :key="version" :label="version" :value="version"/></el-select><el-button text @click="reset">重置</el-button></section>
  ...
  <section class="panel"><h2 class="section-title">Cospec 插件版本</h2><el-table v-if="Object.keys(data.cospecPluginVersions?.byVersion ?? {}).length" :data="Object.entries(data.cospecPluginVersions.byVersion).map(([version, value]) => ({ version, ...value }))"><el-table-column prop="version" label="版本"/><el-table-column prop="runs" label="Run 数"/><el-table-column prop="active_anonymous_terminals" label="活跃匿名终端"/><el-table-column prop="runs_missing_terminal_id" label="终端信息缺失的 Run"/></el-table><div v-else class="empty">暂无插件版本数据</div></section>
