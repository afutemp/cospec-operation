...
const rangePreset = ref<DatePreset>("last30"); const autoRefresh = ref(false);
...
function reset() { rangePreset.value = "last30"; filters.dates = []; filters.agentType = ""; filters.agentVersion = ""; filters.model = ""; }
</script>
<template><div><header class="page-head"><div><h1>运营总览</h1><p>查看 Cospec 工作流的使用规模和资源分布</p></div><div class="refresh"><span class="muted">更新于 {{ query.dataUpdatedAt.value ? new Date(query.dataUpdatedAt.value).toLocaleTimeString('zh-CN', {hour12:false}) : '—' }}</span><el-switch v-model="autoRefresh" inline-prompt active-text="自动" inactive-text="手动"/><el-button :loading="query.isFetching.value" @click="query.refetch()">刷新数据</el-button></div></header>
