...
const agentOption = computed(() => ({ tooltip: { trigger: "item" }, legend: { bottom: 0 }, series: [{ type: "pie", radius: ["46%", "72%"], itemStyle: { borderRadius: 5, borderColor: "#fff", borderWidth: 3 }, label: { formatter: "{b}\n{c}" }, data: Object.entries(data.value.runs?.byAgent ?? {}).map(([name, value]) => ({ name, value })) }] }));
const trendOption = computed(() => { const entries = Object.entries(data.value.runs?.byDay ?? {}); return { tooltip: { trigger: "axis" }, grid: { left: 44, right: 18, top: 20, bottom: 38 }, xAxis: { type: "category", data: entries.map(([day]) => day) }, yAxis: { type: "value", minInterval: 1 }, series: [{ type: "line", smooth: true, symbolSize: 8, areaStyle: { color: "rgba(81,208,177,.16)" }, lineStyle: { color: "#268f7d" }, itemStyle: { color: "#268f7d" }, data: entries.map(([, value]) => value) }] }; });
function reset() { filters.dates = []; filters.agentType = ""; filters.agentVersion = ""; filters.model = ""; }
</script>
<template><div><header class="page-head"><div><h1>运营总览</h1><p>查看 Cospec 工作流的使用规模和资源分布</p></div><el-button :loading="query.isFetching.value" @click="query.refetch()">刷新数据</el-button></header>
  <section class="panel filters"><el-date-picker v-model="filters.dates" type="daterange" value-format="YYYY-MM-DD" start-placeholder="开始日期" end-placeholder="结束日期" /><el-select v-model="filters.agentType" clearable placeholder="Agent 类型" style="width:150px"><el-option label="Codex" value="codex"/><el-option label="Claude Code" value="claude_code"/></el-select><el-input v-model="filters.agentVersion" clearable placeholder="Agent 版本" style="width:160px"/><el-input v-model="filters.model" clearable placeholder="模型" style="width:190px"/><el-button text @click="reset">重置</el-button></section>
  <el-alert v-if="query.isError.value" :title="errorText(query.error.value)" type="error" show-icon class="mb" />
  <div class="metric-grid"><section class="panel metric"><label>Run 数量</label><strong>{{ count(data.runs?.total) }}</strong><small>{{ count(data.runs?.with_parser_facts) }} 个已有解析事实</small></section><section class="panel metric"><label>输入 Token</label><strong>{{ count(data.tokens?.input_tokens) }}</strong><small>覆盖 {{ percent(data.tokens?.field_run_coverage?.input_tokens?.run_coverage) }}</small></section><section class="panel metric"><label>输出 Token</label><strong>{{ count(data.tokens?.output_tokens) }}</strong><small>覆盖 {{ percent(data.tokens?.field_run_coverage?.output_tokens?.run_coverage) }}</small></section><section class="panel metric"><label>使用子代理的 Run</label><strong>{{ count(data.subagents?.runs_with_subagents) }}</strong><small>使用比例 {{ percent(data.subagents?.usage_rate) }}</small></section></div>
